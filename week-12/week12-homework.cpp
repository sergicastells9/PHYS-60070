...
void week12_homework() {
	...
	Double_t mean[arraySize], x_error[arraySize];
	...
	for(int i = 0; i < arraySize; i++) {
		...
		x_error[i] = 1000*(f1->GetParError(1)); // errors too small to be visible
	}
	
	// TGraphError setup
	TGraphErrors* graph = new TGraphErrors(arraySize, mean, energy, x_error, 0);
	...
	for(int i = 0; i < arraySize; i++) {
		...
		y_error[i] = f2->GetParError(2); // get actual y-error here
	}
	...
	res_graph->GetYaxis()->SetTitle("E_{#gamma} - E_{fit}(GeV)");
